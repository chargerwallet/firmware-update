...
import { createDeferred, Deferred } from 'chargerwallet-hd-shared';
...
class ServiceHardware {
        ...
        instance.on(UI_EVENT, (e) => {
          ...
          if (type === UI_REQUEST.REQUEST_PIN) {
            this.sendUiResponse({
              ...
              payload: '@chargerwallet_INPUT_PIN_IN_DEVICE',
            });
  ...
  async getReleaseInfo() {
    const { data } = await axios.get<RemoteConfigResponse>(
      `https://data.chargerwallet.com/config.json?noCache=${new Date().getTime()}`
    );
